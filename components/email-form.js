...
class EmailForm extends React.Component {
  ...
  onInput = email => {
    if (this.state.error) if (validate(email)) this.setState({ error: false })
  ...
  render() {
    const classes = []
    const { ios, safari, error, focus, shaking, defaultEmail } = this.state
    const { flex, loading, errorMessage, message, disabled } = this.props
    ...
    return (
      ...
      >
        ...
        >
          ...
          <Button type="submit" disabled={Boolean(loading || disabled)} width={flex && 175}>
            {message}
          </Button>
        </div>
        ...
        <style jsx>{`
          ...
          .email :global(button) {
            ...
            background-color: ${this.props.flex ? 'var(--accents-1)' : 'var(--black)'};
            color: ${this.props.flex ? 'var(--black)' : 'var(--white)'};
          ...
          .email :global(button:hover) {
            ...
            border-color: ${this.props.flex ? 'var(--accents-1)' : 'var(--black)'};
            color: ${this.props.flex ? 'var(--accents-1)' : 'var(--black)'};
            outline: none;
          }
          ...
          .focus label {
            ...
            border-color: ${this.props.flex ? 'var(--white)' : 'var(--accents-8)'};
          }
          ...
          input {
            ...
            color: ${this.props.flex ? 'var(--white)' : 'var(--black)'};
            display: block;
            text-align: 'left';
            font-size: 14px;
